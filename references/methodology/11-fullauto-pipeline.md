...
**危害三问内联(01 §3.5)**:差分通过标 confirmed(tech) 的同时,hunter 必须在台账 notes 答危害三问(谁能打/打到什么具体物/防线在哪)——写不出具体物只能 severity=info-pending;批量反驳者子代理在队列项边界/出口检查时复核(01 §3.5.3)。
...
**标准动作(第 15/16 轮校准)**:①基线=攻击=对照全零差分时,**必须先查已采证据里该参数的真实形态**(页面链接/表单字段,防 NEXT-ROUND 交接假设错参数名——实测 id= vs pid= 教训);②再打一发**消费性判别**(不存在的值)区分"参数未消费"vs"数字归一",二者都 clean 但结论写法不同;③判读已存证据的 grep 一律带 `-a`——GB2312 证据被当二进制会静默吞掉全部匹配(第 16 轮自动校验循环再次踩中,此规则**内建于管线校验步骤**,不依赖记忆,09 §1.1);④**写类探针正对照先行**(第 2 轮 testfire 实测):跨用户写第一发表单重渲染差点误判"未执行"——先打一发自有账户正对照拿到该端点的成功响应格式(postResp 等),再回头判读跨用户响应,成功消息可能藏在同构格式里
...
4. **盲复现门闩(可选启用,默认关闭)**:把 confirmed 项写成**零上下文《AI 待复现报告》**——自包含:目标/授权依据/公开测试账号/逐步请求(含完整 body)/每个 payload 的判定标准/**环境前提与陷阱(限流窗口与 0B 语义/响应编码与 grep 纪律/验证码等 trick 的会话前提/端点级怪癖)**/超时与重试策略/安全约束与副作用声明/已知不可复现项,不依赖任何会话记忆 → 交零上下文执行者复现(**执行等级**:subagent 全盲首选;并发受限时降级为按报告字面的机械脚本执行,结果文件必须标注执行等级;**执行提示词模板**:`references/templates/ai-repro-executor-prompt.md`,替换报告路径与结果路径即用)→ FAIL 或执行者提出报告歧义 = **报告缺陷**,修正重测后才准出终稿。**危害双门(01 §3.5.3)**:盲测执行者同时做危害合理性评估(复现性+危害性一次做完);批量反驳者子代理在出口检查先行一轮,本报告门是终审。模板:`../templates/adversary-reviewer-prompt.md`。报告书写两条铁律随之固化:①每个攻击 payload 后必须跟显式验证步骤(testfire F-03 验证步骤隐含省略致盲测误判 FAIL);②超时 15s 允许重试 1 次再判 FAIL(单次网络抖动不应埋葬真漏洞);③同族多端点的**每一小节都自带基线步骤**,不复用其他小节的(F-21 盲测缺基线行,执行者被迫自行补测);④预期观察值写**当前实测值**并注明测量时间,不写历史记忆值(F-21 预期字节数系统性偏大 300B,历史残留)。**环境前提章节的价值**:aiwadongdemumu 报告的成败完全押在"0B 窗口语义/验证码 trick 的无码 session 前提/GB2312 grep 纪律"三件事上——不写,盲测执行者必踩坑,所有真漏洞都会被误判 FAIL. **范围与默认(20260831 用户指令)**:盲测范围=**反驳者受理的项 only**——驳回/clean/rejected 项不是漏洞,盲测验证的是"漏洞可复现性",给它们跑盲测是浪费额度(R18 实测给 4 项 clean 安排盲测属纯浪费,已裁撤);**默认关闭**,启动条件=用户明示要求 / findings 将进正式平台提交;启动方式见 README「零上下文盲测」节。
