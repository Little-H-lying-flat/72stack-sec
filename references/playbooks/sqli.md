...
# 同 CMS 数字参数族:框架级校验"一死全死"(mlecms detail/ad/hits 全 int 校验),
# 一个端点确认可注入则同族兄弟参数是低本高概率复测对象
# (20260830 实测:Show.pkid 确认注入 → 兄弟位 lipinshow.id 即中;detail 硬化 → ad/hits 同硬化)
```
...
#### ASP 经典错误 → 判定速查（20260830 实战回填）

| 错误 | 含义 | 判定 |
|---|---|---|
| `VBScript 运行时错误 '800a000d'`（Type mismatch） | 值在 VBScript 层被强转 | **转型发生在进 SQL 之前 → 该参数 clean**，别再烧请求 |
| `ADODB.Field 错误 '800a0bcd'`（BOF/EOF） | 查询执行了但零行 | 值是**引号内字符串上下文**整体进 SQL → 突破需要引号，先测过滤器解码行为 |
| 统一"非法操作"弹层页（固定小体积） | 应用层关键字黑名单 | 立即测 `%27`：也被拦 = 过滤器**解码后匹配**，URL 编码绕过失效；只拦 raw 则有戏 |
| 布尔对行为**倒置**（1=1 报错 / 1=2 正常空） | 值先进了 VBScript 或过滤层，不是直接进 SQL | 按上两行重新归因，别当"疑似注入" |
| `80040e21`（ODBC 不支持所需属性） | 空参数畸形 SQL **或** 游标/驱动拒子查询形状 | 非注入信号：先测无参基线归因；子查询各形态全中此错=数据抽取死路（20260830） |
| 响应=部分渲染+尾部错误（如商品页+行190错误） | 同页多条 SQL，后续查询因参数来源空而挂 | 判 TRUE/FLASE 看主体内容（title/商品块），不许"有错误字样=失败"一把抓 |

> 实测来源：5vshop ASP 商城（同一站上三类签名同日集齐）。过滤器解码判定 + 800a000d 强转判定，两次都把"疑似注入"在 2 个请求内定性。
