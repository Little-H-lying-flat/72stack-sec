...
- **工具映射**：`view`→Read 工具；`grep`→Grep 工具或 Bash `grep`；`create`/`edit`→Write/Edit 工具。文件路径一律用绝对路径。
- **MCP 预扫合并**：若编排器告知 `raw_*.json` 含 `source: "mcp:scan_sensitive"` / `mcp:cloud_scan` 标记的条目（来自 first-miniapp-debugger MCP），与分析脚本产出同等对待，同样要过误报过滤，不因其来自 MCP 就直接采信。
