...
## v2 适配注记（ZCode + first-miniapp-debugger MCP 整合版）

- **工具映射**：`view`→Read；`grep`→Grep 或 Bash `grep`；`create`/`edit`→Write/Edit。目标类型 `burp_info` 在本版扩展为通用 `external_info`（Burp 抓包、MCP `miniapp_cloud_captures` 捕获的 callFunction 真实参数、`miniapp_get_storage` 导出的存储数据、`miniapp_http_request` 的请求响应记录，均可作为外部情报传入），分析方式不变：**代码层面验证和关联**。
- **外部情报给到即用**：编排器传入的 external_info 必须完整使用，严禁只关联其中一部分。
