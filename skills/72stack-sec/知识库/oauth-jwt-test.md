...
假点：none 只本地解码、服务端仍验签；RS→HS 服务端拒 HS；kid/jku 白名单；弱密钥字典没命中；票过了但 me 仍是自己且换 uid 无效；库报「验签成功」但只确认解码/填充形态、公钥完整对照未跑（见下「验签半截」）。单站没中不删上节手法。

### 验签半截（假点差分，不单开短表开场行）

认：JWT/RSA 验签路径在「解码成功 / 填充形态 OK」就返回成功，**没有**用公钥做完整密码学对照。

打：同一口对照——(a) 真钥真签 (b) 乱改 payload 未重签 (c) 假钥重签 / 空签 / alg=none。若 (b)(c) 也被标「验签成功」且业务认身份 → 记差分；否则假点。

假点：走完整验签路径，伪造即失败；该库/中间件根本不负责验签（只解析 claims）。通用伪造枪仍听 `hunt-iter` ②.1，不另开短表手法行。
...
认：token **交换/铸造**口（`exchange` / `mint` / `assume` / `token-service` / `grant_type=*`）只验入票签名与 issuer，**不强制**原票的 scope / 权限声明。低权甚至内部匿名票可被换成管理 scope；日志里还常挂着匿名身份。

打（不登录或只持低权票）：
1. 对照：原票 scope 只有 read/anonymous；交换后票解码看 scope/roles。
2. 用换出的票打管理 me / 用户列表 / 写口只读差分（≤5 组）。
3. 假值：乱改签名的入票应拒；同权交换不算升权。

算成：换出票带原票没有的管理 scope，且业务口认了（身份或他主体列表）。

假点：换后 scope 仍同权；高权声明业务一律 401；根本没有交换/铸造面。与「刷新票空包发管理员」相邻但不同：那是空包直接出 admin，本条是**有入票却未强制原 scope**。
