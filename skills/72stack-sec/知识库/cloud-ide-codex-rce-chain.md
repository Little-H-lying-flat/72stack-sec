...
## 终端/PTY WS 漏鉴权（短表开场有指针）

认：同产品里 HTTP 或业务 WS 走了 `validate_auth` / `@requires`，终端/PTY/shell 的 WS 直接 `accept`（中间件把失败标成未登录用户仍握手）。笔记本/编程台换皮照认，不钉产品名。

打：对照有鉴权的那条 WS。无票连终端口，看是否握手并出现命令面。只打当前站。

算成：命令跑起来（无害标记）。

假点：终端口同样要票；失败即关；没有 PTY/命令面。
