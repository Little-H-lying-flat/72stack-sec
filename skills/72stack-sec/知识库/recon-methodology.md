...
>
> 调搜用 MCP `fofa`（`fofa__get_alerts`），不要自己 curl。Key 在 MCP `fofa` 注册配置的 `env`（FOFA_EMAIL/FOFA_KEY 等六项；注册方式见本 skill `README.md`）：主号 → backup → backup2，`fofa.py` 遇 429/820041 自动切。限流闸认 `dig-scope` §2.1.4。**禁止**把 email / key 写进本文件或对话。
