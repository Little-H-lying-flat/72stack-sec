...
认：集群 join / 节点互通 / 对等验签的共享密钥配置为**空**时，实现仍把空串当有效 HMAC/共享钥（或确定性派生），未登录或未持节点身份就能过 join/互通口。

打：
1. 找 join / peer / cluster / interop 验签口；配置空或默认空密钥文档/环境。
2. 空密钥对照：按文档用空串派生签名打 join；非空乱密钥应拒。
3. 过了再看能否冒充内部节点身份读名单/下发配置（只读差分，≤5 组）。

算成：空密钥验签通过且业务认成内部节点（出节点身份或内部配置面）。

假点：空配置直接拒连；必须显式非空密钥；join 口只内网且双向互证。单站没中不删短表这行。
...
场景：同一产品把鉴权拆成多条作用域（如 Admin Filter vs Open Filter），由注解字段决定走哪条；**默认作用域偏开放**，管理写接口漏标就会掉进「默认关鉴权」的那条。

认法（形态，不绑死产品名）：
- 文档声称「已开管理鉴权」，但用户/角色/权限类写口未授权仍通。
- 对照：明确标成管理面的读/写口未授权被拒，同功能另一组口却放行。

开场：
1. 未授权打管理对照口（应拒）。
2. 未授权打用户/角色/权限创建类口。
3. 若禁内置管理员角色名，改自建角色 + 通配资源再登录验证。

假点：所有作用域开关都已强制认证；漏标已修；外网无写口仅控制台壳。

例（便于认版本，不是短表点名清单）：Nacos 3.0.0–3.2.3 `@Secured` 默认 `OPEN_API`，用户/角色/权限控制器漏标 `ADMIN_API`（修于 3.2.4）。

### 豁免正则打完整 URI（短表有指针）

场景：OAuth2-Proxy `skip_auth_routes`、低代码 webhook 豁免等，用正则决定「这段不用登录」，但匹配的是 **path+query 的完整 URI**，且模式未锚定。

认：配置/源码里豁免正则；任意带 query 的 URL 仍进同一匹配函数。
打：受保护口后拼 `?/…能命中豁免的片段`。对照纯 path。
假：只匹配 path；`^$` 锚定；query 在匹配前被剥掉。
