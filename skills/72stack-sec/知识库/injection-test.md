...
> 短表「列表筛选项 OR + total」「邮件订阅 iframe 同目录 list」「写口入库、读口才拼 SQL」用标题搜。WooYun 统计 / sqlmap --os-shell / 反弹 shell / 英文附件已砍；现场按栈自己变，不靠教材。
...
### 写口入库、读口才拼 SQL（二阶 · 短表有指针）

认：写口能把脏数据存进库（注册昵称、改资料、留言/工单备注、导入 CSV、问卷开放题），**当时回包不炸或只当普通字符串**；另一条读口/登录口/列表口/导出口/后台审阅口/定时任务才把该字段拼进 SQL。和「当前请求立刻布尔差分」不是同一枪。

打：

1. **圈写口与触发口。** 写：`register` / `profile` / `comment` / `feedback` / 导入。触发：登录、`/me`、搜自己、导出、管理端打开该对象、定时报表。JS/抓包里「先写后读」同字段名优先。
2. **先对照、再探针。** 同一写口先后写入布尔假 / 布尔真（或带唯一 marker 的一对），再强制走同一触发读口。比的是**两次触发回包**的长度/条数/报错差分，不是写口当时 200。
3. **探针要轻。** 先 marker + 布尔对；有差分再考虑报错/延时。列表类触发口 pageSize 保持小。**本枪不用** sqlmap `--dump` / 全库 OR；出他主体或稳定差分就停。
4. **位置。** query 没吃改 json body；昵称没吃改备注/隐藏字段；登录触发没差改导出/后台详情。

算成：触发口出现稳定布尔/报错/条数差分，或读出口径里出现**他主体**数据。只证明写口存进了字符串、触发口仍当普通文本 → 没成。

假点：写口本身就一阶注入（那是当前请求那枪，别标二阶）；触发口预编译/参数化，脏数据只回显不当 SQL；只有存储 XSS 没有 SQL 差分；必须管理员手点才触发且你够不到触发面。单站没中不删短表这行。每个写字段丢 `'` 不当二阶进度。
