...
| 打 MCP server / agent skill / 工具供应链 / agent 自身基础设施 | `15-mcp-agent-infra.md`(工具投毒 / confused deputy / 传输层) |
...
单条手工探针打完本文件场景后,要批量测(变异/生成式/遗传三策略、系统提示词泄露探针族、GA 代数上限、oracle 机械判定)→ Read `../../methodology/13-interface-fuzzing.md` §4;红线:只打目标自己的 LLM 集成接口,不指向第三方模型 API。
