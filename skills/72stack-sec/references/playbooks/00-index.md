...
> **国内默认弹药在知识库，不要卡壳直读本目录撞型 playbook。**
> 撞型对照：`知识库/同型对照.md`。卡壳仍先开知识库对应文件；本目录只在「playbook 独占」或用户点名 / 靶场 Phase 确认时 Read。
> 独占：SSTI / 框架 RCE / 云 / SAML / APK / 内网后渗透（点名）。
...
| 🔴 P0 | `rce/00-index.md` | Log4Shell / Spring4Shell / Fastjson / Struts2 / 命令注入指纹库(子目录:framework / cmd-inj / deserialize / file-chain / ssti / xxe / supply / proto-pollution) |
| 🔴 P0 | `file-upload/00-index.md` | 解析漏洞 + 编辑器漏洞 + 截断绕过 |
| 🔴 P0 | `path-traversal/00-index.md` | `../etc/passwd` 加 6 种编码 + WEB-INF / web.config |
| 🟠 P1 | `info-disclosure.md` | .git / .svn / 备份文件 / phpinfo / 日志 / OSS bucket |
| 🟠 P1 | `logic-flaws/00-index.md` | 密码重置 4 模式 / IDOR / 越权 / 验证码 / 支付 |
| 🟠 P1 | `arbitrary-x-authz.md` | 任意 X 子授权（任意账号 86.4% / 任意操作 72.5%）|
| 🟠 P1 | `oauth-saml-jwt/00-index.md` | redirect_uri / state / JWT alg / kid / SAML 包裹 |
| 🟠 P1 | `sqli.md` | 27,732 个真实案例提炼，含高频参数频率表 |
| 🟠 P1 | `ssrf-cache-host/00-index.md` | 内网探测 + 云元数据 + Host header / 缓存投毒 |
| 🟡 P1/P2 | `api-rest/00-index.md` | BOLA / Mass Assignment / 速率 / CORS |
| 🟡 P1/P2 | `graphql.md` | Introspection / 嵌套 IDOR / DoS |
| 🟡 P2 | `race-conditions.md` | 优惠券双花 / 余额超扣 / 限额绕过 |
| 🟡 P2 | `xss/00-index.md` | 7,532 真实案例，上下文绕过表 |
| 🟡 P2 | `http-smuggling.md` | CL.TE / TE.CL / H2→H1 |
| 🟡 P2 | `mobile.md` | 安卓导出组件 / Intent / WebView / Pinning |
| 🟡 P2 | `llm-prompt-injection/00-index.md` | Prompt 注入 / RAG 投毒 / Agent 工具 |
