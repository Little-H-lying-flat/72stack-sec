...
> **双重假象陷阱（20260830 实战）**：改密表单带 `username` 字段 ≠ 服务端使用它——实测一例服务端完全忽略 `username` 只改会话用户：跨用户+错旧密报"旧密码不正确"、跨用户+自己旧密报"成功"，看似身份混淆 ATO，实际两次"成功/报错"都以**会话用户**为准。判据只有一条：用新密码登录**目标**账号，成=洞，败=字段被忽略。响应回显不算数。
...
【操作】新 cookie jar 只访问表单页、**绝不请求 GetCode**，提交 `verifycode=` 空值——空串 == 空会话，注册/登录双口直过；**适用面=同站所有 verifycode 口逐口复测**（实测找回密码第一步同样直过），另注意"同站部分口无验证码"的不一致本身就是爆破面
...
> **老 ASP 商城实战注记（20260830，5vshop）**：金额常不止 `price` 一个字段——整单聚合值 `total`/`allmoney`/`money1` 全是**结算页客户端隐藏字段**直提 pay_ok.asp，改聚合值即改订单落库金额（实测 1399 元商品记成 0.01 元）。证据用**双单差分**：先原价正常下单做基线，再同流程篡改聚合字段下单，订单列表并排对照金额。结算链被 JS 改写语义时（getJSON/字段重写），直接真浏览器走通到支付页，`evaluate` 改隐藏字段后 `form.submit()`，DOM 表单即字段真相。
