...
报告/{host}_{类型}_{日期}.md   # 打穿才写,按 ~/.grok/rules/vuln-report-format.md 两张表
js/{host}/                     # 全部 JS
```
**DONE.md 必含两行自证（缺行=主控驳回）**：
- `短表核对:对号 N 条 / 命中 M 条`（开场几枪的执行凭证）
- `细则核对:已读(长战役,注明 §1.1.1/§2.1/§4.3 命中情况) 或 N/A(短平快站)`（长战役 Read `~/.grok/rules/dig-scope-workflow.md`，禁止读 skill 内 stub 当正文）

长战役细则（懒加载）：扩面卡壳 / 优质根域回灌 / 反空转 / 覆盖率 / 禁偏科 → Read `~/.grok/rules/dig-scope-workflow.md`。短平快站 N/A，不要预加载。
