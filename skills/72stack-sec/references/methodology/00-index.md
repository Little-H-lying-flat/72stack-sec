# ⚠ 国内 SRC 主流程封条（乌鸦 · 2026-09-10）

> **禁止**把本目录（`references/methodology/`）当第二套主库通读。  
> Grok-only 国内 SRC：**只走** `D:\dsh-72stack-sec\skills\72stack-sec/SKILL.md` → `知识库/打穿短表.md` → 显式指针模块。  
> 本目录文件**仅当**短表/模块/SKILL **写出相对路径指针**时才打开对应单篇。  
> 与 `D:\dsh-72stack-sec\rules` 或 `SKILL.md` 冲突时，以 rules / SKILL 为准。  
> 发现引擎正文若需语义审计：优先 `14-semantic-audit.md`（显式指针），不是通读 01–13。
## ARCHIVE 声明（P1 · 2026-09-10）

除短表/模块/SKILL **显式指针**到的单篇（常见：`14-semantic-audit.md`、`03-evidence-discipline.md`、`11-fullauto-pipeline.md`）外，本目录其余篇对国内 SRC 主流程视为 **ARCHIVE**：可按指针打开，**禁止**当开场通读或第二主库。


---

# src-hunter 方法论入口

> 视角：黑盒 SRC / 众测 / 漏洞赏金（H1、Bugcrowd、补天、CNVD）
> 假定：攻击者只有 URL、参数、HTTP 抓包能力，**没有源码**

---

## 这套方法论怎么用

猎手的工作流被切成 4 段，每段对应本目录一个文件：

```
[01] 选靶——决定先打哪里（攻击优先级）
        │
        ▼
[02] 起手——构造 payload，带 bypass 矩阵（绕过工具箱）
        │
        ▼
[04] 控制缺口（Control Gap）狩猎——把"敏感操作 ↔ 应有控制"翻译成黑盒探测策略
        │
        ▼
[03] 收尾——证据纪律：抓包、回显、带外、复现率，不写"我猜的"漏洞
```

| 文件 | 作用 | 读它的时机 |
|------|------|-----------|
| `01-attack-priority.md` | 决定 RCE > 文件写 > 鉴权绕过 > 注入 > 信息泄露的报告顺序，量化 P0–P3 评分 | 选目标 / 排报告优先级时 |
| `02-bypass-toolkit.md` | SQLi、XSS、命令、路径、SSRF、WAF 的通用绕过决策树 + 编码字典 | 任何 payload 被拦截时 |
| `03-evidence-discipline.md` | 黑盒证据纪律：HTTP 包、回显、DNSLog、Diff、复现率；如何避免"我以为"漏洞 | 写报告之前 |
| `04-control-gap-hunting.md` | 把 9 类敏感操作（数据修改/批量/权限/资金/SSRF/文件/命令/认证/越权）翻译成"哪些控制应该存在、如何探测它缺失" | 拿到一个新功能、不知道从哪入手时 |

---

## 与 SRC 平台的对齐

不同平台对"算分要素"的偏好：

| 平台 | 偏好的漏洞类型 | 必须有 |
|------|---------------|--------|
| **HackerOne** | RCE、Auth Bypass、IDOR、SQLi、SSRF | CVSS 3.1 或 4.0 + 可执行 PoC + 影响范围 |
| **Bugcrowd** | VRT 分级（P1–P5），偏好可链接的攻击链 | 步骤可复现 + 截图/视频 |
| **补天 / CNVD** | 通用型 RCE、SQLi、未授权、敏感信息泄露 | 漏洞证明（截图、复现包） |
| **众测/HVV** | 内网入口、控制权、横向 | 影响资产清单 + 后利用证据 |

**统一规则**：
1. 只测授权资产，不外溢
2. 不删/不改/不批量拖库，证明即止
3. 数据脱敏（用户名、手机号打码）
4. 不上传/不留 webshell；命令执行只跑只读命令（`id`、`whoami`、`uname -a`）

---

## 价值排序（精简版）

| 等级 | 漏洞类型 | 平均赏金区间（参考 H1 / Bugcrowd） |
|------|---------|---------------------------------|
| 顶级 | 未授权 RCE / SSRF→RCE / 反序列化 | $5k – $50k |
| 高 | 鉴权绕过 / 公开 IDOR 大面积数据 / 任意文件读 | $1k – $10k |
| 中 | SQLi / 越权 / 文件上传（受限） | $300 – $3k |
| 低 | XSS / 信息泄露（无敏感数据） | $50 – $500 |

> 这个顺序就是 `01-attack-priority.md` 里"攻击路径最短原则"的现金体现。

---

## 配套 playbook 目录

漏洞类型分类详见 `../playbooks/00-index.md`。
