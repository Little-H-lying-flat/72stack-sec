...
**会话状态持久化（跨会话续作必备）**：测试账号(用户名/密码/注册方式)、cookie jar 文件名、已知资源 id 清单(订单/留言/商品)记在 `scope.md` 的 `next` 节——只存响应不存请求体的证据会导致续会话无法恢复登录态，被迫重新注册污染数据（20260830 第 8 轮实测教训）。**续会话先验账号存活**：演示/靶场环境的库可能轮间重置（20260830 第 12 轮实测：mlecms 上轮注册号轮间消失致 4 请求白烧，5vshop 同日不重置）——续作第一个请求先登录旧号或查旧资源 id，失存即重建并在 scope.md 标注。
