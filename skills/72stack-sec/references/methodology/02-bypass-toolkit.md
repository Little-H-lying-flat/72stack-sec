...
> **先画像，再选技**：拿单关键字探针摸黑名单边界（如 `id=1 select` / `id=1 union` / `id=1 from`，各一请求看拦截页 vs SQL 500 vs 正常），再从下表挑对策——实测案例（20260830，5vshop ASP）：黑名单仅 `select`+`'`（大小写不敏感、解码后匹配，%27/SeLeCt 均拦），`union/from/where` 全部穿透到 SQL 层；不画像就盲试本表会浪费一半请求。
