...
## 2.5 命名规律参数 fuzz(接口无参数时的默认动作,20260831)

端点存在但无可见参数(如 /user/info)→ 按**命名规律族**构造参数候选,有序发送(不是乱炸):

```
id 族:      id, userId, user_id, uid, no, num, code
对象族:     {资源名}Id / {资源名}_id   (user/info → userId|user_id)
通用族:     name, key, type, page, size, q, keyword, sort, order
管理员族:   admin, isAdmin, role, level, groupId
```

规则:
- 命中 oracle 同 §1 参数名 fuzz:加参 vs 基线的响应差分(消费=变化)
- 命中一个即停本族,记台账后按该参数语义走三问(14 §1.1)
- 批内预算:每端点 ≤1 族完整尝试;全部零差分 → 端点记"参数未知",不硬磨

## 2.6 规模化纪律(数量决定产出,20260831)

JS 面的规律:**量级差一个数量级,产出差一个战役**——117 个 JS 可能白干,11897 个 JS 里才出 16 个真洞。含义:

1. JS 收集**不设上限先收满**(07 三模式全开),阶段内不下"没有就没有"的结论
2. 端点清单先穷尽**再**语义审计——审计对象不足时三问再准也没靶子
3. 产出预期管理:真洞率 ~0.1%(16/11897)是常态,**别为低命中率沮丧,为覆盖率不足 alarm**
4. 大目标 = 08 多 agent 天然适用场景(面大才能分片)
