...
| **签名搜索命中业务数据** | 搜 SQL 错误签名命中演示/业务数据（如下载文件名 `test_download_mysql_fix`） | 签名命中 ≠ 错误——先核对命中行的上下文再定性（20260830 实测） |
...
> **探针构造也要"流量原貌"（20260830）**：老站复现先读页内 JS 再发请求——`<form method="post">` 可能被 `$.getJSON()` 改写成 **GET**，`guigechoose()` 之类 JS 会**重写字段值**（实测把 gid 改成规格值、gidget 固定 '1'），`<input type="image">` 提交时带的是 `按钮名.x/.y` 坐标对。只按表单静态标签构造，服务端分支对不上 → 静默 302/无效果，白烧一轮。JS 语义过于复杂时（多级结算/重写链），**真浏览器走通到目标步骤**，页面 DOM 表单即字段真相，再 `evaluate` 改值提交做差分——比盲猜快一个数量级（第 9 轮 5 探败于盲猜，第 10 轮浏览器 4 步打通）。
...
**去重抑制**：同一 endpoint + 同一漏洞类的攻击变体，差分确认一次即收敛为一个 finding，后续同源变体不再重复触发——重复打不会增强证据，只会触发风控 / 封号。
...
### 3.1 写越权最小伤害序列(20260831 吸收 clown,修正"只读不写"的过度保守)

**写 IDOR/越权是高危面,不测=漏报。** 红线不是"不写",是"最小伤害地写":

1. **先添加**:创建挂到别人名下的资源(订单/地址/收藏 id 换成他人)——添加是新对象,不碰既有数据
2. **再删除自己刚加的那一条**——清理且只动自己的
3. 没有创建口、只能动现成对象时:只改**自己能改回去**的测试字段,打一次,改回
4. 改密/改角色/改绑:可探(改完立刻改回);改不回就停在回包证据,不留用户号的变更
5. 永不做:删他人既有数据/批量/真资损/把用户号改死

**禁止把"只读红线"理解成"写 IDOR 不用测"**——红线的本意是伤害最小化,不是回避整个高危面。
...
**限流陷阱**：连续快速请求会撞站点限流窗口（响应 0B / 超时）——**0B ≠ 差分不成立**。复现请求间隔 ≥8s；一旦出现 0B 窗口，先等 30s 降速重测再下结论（实战教训：同一布尔对在限流窗口内全 0B，降速后 3/3 稳定差分）。
