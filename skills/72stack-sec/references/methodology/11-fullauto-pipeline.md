...
| 信号(滑动窗口) | 当场动作 |
|---|---|
| 连续 3 发同端点同形失败(同字节错误页/同 403) | 该参数/端点熔断,跳下一项(不再等预算耗尽) |
| 响应出现异常模式(字节数与基线偏差 >10x / 新增 Set-Cookie / Server 头突变) | 暂停 10s,复核是否环境突变(env 重置/换后端),是则按 §4.5 处理 |
| 探针 URL 疑似越出 mission in_scope(重定向带外域/参数回显外域) | **立即停该链路**,记录,不跟随 |
| 我方载荷可能触发目标异常(500 风暴/连接reset 连发) | 降档节流,连续 2 次仍触发→该资产挂起转监控(§4.4) |

哨兵动作全部**当场执行并记 suspects/台账**,不等队列项收尾——这是"热路径"与门闩(阶段边界)的区别。
...
2. **归因**:先查自己——对照 03 §2 幻觉表(响应特征幻觉 / 限流窗口假阴性);再查目标——对照 02-bypass §2.1 过滤器画像(被什么规则拦)
   **元认知两问(必答,20260831)**:①我在用"最常见"的方法吗?——越常见越被防,常见打法被拦是常态不是例外;②开发者认为"不可能"的操作在哪?——他们觉得没人会这么调的接口,就是突破口
