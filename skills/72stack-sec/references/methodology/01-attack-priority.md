...
## 3.5 危害定性门(反通胀)——confirmed ≠ 有危害

> 视角:`confirmed` 只证明**技术上可复现**,不证明**有危害**。差分成立就标 High,是 AI 挖洞最大的通胀源。本节是 candidate→confirmed 之间的第三道门。

### 3.5.1 危害三问(每个 confirmed 必答,一句话版写进台账 notes)

1. **谁能打?** 未授权 / 普通注册用户 / 特权用户——攻击前置成本决定起点
2. **打到了具体什么?** 必须说出具体物:哪张表/哪类数据/多少资金/什么规模/能否持续——**写不出具体物 = 自动降 info**
3. **防线在哪?** 前端校验不算防线;网关限流/风控/监控是补偿控制,如实写;**by-design / 文档声明接受 → 标 `by-design`,不进 confirmed 统计**(靶场 intended challenge 同此)

### 3.5.2 反通胀下限表(只做到左列 = 等级上限只能取右列)

| 只做到这些 | 等级上限 |
|---|---|
| 布尔盲注仅 oracle(未证明库里有敏感数据) | Low |
| 反射 XSS(无受害者场景) | Low/P3 |
| 信息泄露(说不出泄露内容对谁有害) | Info |
| 验证码绕过单口(无组合影响) | Medium |
| 越权/逻辑(仅"能调通",无副作用量化) | Medium |

### 3.5.3 零上下文反驳者(批量,三层)

- **L1 内联**:candidate→confirmed 转换时 hunter 自答三问,写台账 notes(零成本,源头拦通胀)
- **L2 批量反驳**:队列项边界/出口检查时,**一个子代理吃整批** confirmed(三问答案+证据路径),逐项独立作答三问并裁决:**受理 / 降级 / 驳回**;红线="你没参与挖掘,不要客气;写不出攻击者具体拿到什么就驳回";裁决零来回——驳回→status=rejected(理由留痕),降级→更新等级(原值留 notes)
- **L3 报告门**:AI 待复现报告执行者同时做危害合理性评估(11 §7-4 双门)
- 提示词模板:`../templates/adversary-reviewer-prompt.md`

---



## 3.6 链式升级——把每个洞榨干(20260831)

confirmed 不是终点。每拿到一个洞,问:**这一步能链到什么?**

| 起点 | 升级链(逐跳问"下一步拿到什么") |
|---|---|
| SSRF | → 云元数据 → AK/SK → 对象存储列桶 → 内网服务 → RCE |
| 任意文件读 | → 配置文件 → DB 凭据 → 数据库 → admin hash → 后台 |
| SQLi(oracle) | → 敏感表证实 → 拖样本 → 写通道?(FILE/堆叠) |
| 存储型 XSS | → 打谁?(admin 后台/客服端)→ 会话窃取 → 越权操作 |
| 未授权 API | → mass assignment → 改权限字段 → 提权 |

规则:每跳都要新证据+新差分(03 纪律),链上每一环独立记台账(`src:` 因果链引用上级 finding),**危害定性按链终点算,不按起点算**。

## 3.7 Critical 目标导向(20260831)

正式档(formal tier)战役设定**最低产出目标:≥3 个 Critical 级**方向清单——RCE / SSRF(云元数据)/ SSTI→RCE / SQLi(可拖)/ 任意文件读 / 认证绕过 / 批量 IDOR。语义审计(suspects)分优先级时,**能链向这份清单的面优先**;收官时 Critical 清单逐项标注"已拿/链断在哪/为何不可达"——目标清单就是时间盒的分配依据。

> 练习档(practice)不强制,但 suspects 分优先级时同样按"能否链向高危"排序。
