...
**序列**:MCP `http://127.0.0.1:4554/sse`（`list_packages` → `decompile` → 静态扫）拿接口/密钥/appid → 本 skill 短表+知识库打后端（常漏鉴权）→ 支付/兑换/IDOR。`web-view`/云开发细节才读 miniprogram-hunt 对应 playbook。
