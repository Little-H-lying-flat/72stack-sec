...
1. **取 scope**:搜该 SRC 公告/官网规则页圈范围(全资域名清单、规则、时间盒);拿不到公告就以**备案根域 + 主品牌域**为 scope,参股/蹭域名默认不挖;出 scope 立即停(硬约束 4)。**开工同时写标记 `%USERPROFILE%\.agents\.dig_active\<任务根名>.flag`(内容 = 任务根绝对路径;每任务一个 flag,并行多 SRC 互不覆盖)——Stop hook 闸门,收口删自己的 flag 才放行(hook 仅 Claude/ZCode 环境,Grok 无 hooks)**。
...
5. **报告收口**：confirmed 漏洞按 `~/.grok/rules/vuln-report-format.md`（两张表，唯一报告格式）写报告落 `报告/`；**没打穿也要收口短报**（做到哪/落了什么/下一步），不带问句停工。**两段式收口：有 confirmed → 先出阶段收口短报，再进复现阶段（证据固化→存活复测→升链一轮→报告全落盘），复现完成才删自己的 `.flag`；无 confirmed 且只剩挂起项 → 挂起项写进队列「挂起」节后删自己的 `.flag`。用户明确叫停 → 立即删自己的 flag 停工。**勿动其它任务的 flag**。**高危已落盘 → DONE 末尾写 `拟进:认<形态>打<打法>` / `拟补:<哪行>+<新差分>` / `不进:<原因>`（中危不拟进）。
