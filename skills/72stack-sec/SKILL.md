...
5. **报告收口**:confirmed 漏洞按 `~/.grok/rules/vuln-report-format.md`(两张表,唯一报告格式)写报告落 `报告/`;**没打穿也要收口短报**(做到哪/落了什么/下一步),不带问句停工。**收口后、或用户明确叫停时,删除 `.dig_active` 标记再停**。高危已落盘 → DONE 末尾写 `拟进:认<形态>打<打法>` / `拟补:<哪行>+<新差分>` / `不进:<原因>`(中危不拟进)。
