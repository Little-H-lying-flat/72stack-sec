...
description: |
  ...
  WHEN：挖 XXX SRC / 某品牌 / 挖域名 / 开线程 / 继续挖 / 有号挖 / 只未登录 / 自动注册 / 账号测试 / 用户点名本 skill / 白盒源码审计（Linux Kernel / Chromium / Laravel / Spring / Django） / 复盘模式（证据包对照、假 covered 抽查、闸口演练；禁探口）。
...
> **术语表（单真源）：** 自造词释义只认 [`术语表.md`](术语表.md)。SKILL/调度/线程必读只引用，禁止另写一版。



> **开场指针（单轨）：** `知识库/打穿短表.md`（先读「开场加打 · 认法层」；对得上才开枪，假点停，禁止扫表凑枪/当百科）→ 对得上再开 `知识库/<模块>.md` → 立法冲突以 `~/.grok/rules` 为准。  
...
| 派轨 / 空席补席 / 一种子闭环换种 / 升链席（读证明、≤5、禁真写） | 无人值守跨租户试探、批量拖会话仓 / 令牌集 |
...
### 复盘模式（证据包 · 禁探口 · 优化 skill 用）



> 何时用：攻防演练里执行方**不能探口/写 PoC**，或用户只要闸口/假 covered 对照。产品目标仍是发现引擎可审计，不是绕开红线去打。



**输入（证据包，由靶场方提供，不含源码、不含可复现攻击步骤）：**

- 公开范围文档（公告 / scope）

- 预置 `{host}/endpoints*.md`、`DONE_anon|DONE_auth`、`suspects.md` 差分样例

- 对照页：每类「该记 / 该拦 / 假 covered 样例」



**只做：**

1. 读对照页 + 样例 DONE/suspects/endpoints  

2. 跑 `suspects_coverage_check` → `done_fields_check` → `p2_gate`（默认含盲区头严格闸 + `write_surface_check`）→ 有报告则 `report_format_check` → `blindspot_remind

3. 若是差分证据包：另跑 `python scripts/evidence_pack_check.py --pack-dir <lab.ok|任务根>`（验 scope/diff/conclusion/指回；「不写利用步骤」否定句不误杀）`  

3. 对照清单输出：真阳性（该记且样例已写）/ 假 covered（样例违规且闸该拦）/ 立法缺口（该拦但闸滑过）  

4. 优化项只回灌 skill 立法/脚本，不扩 payload、不探口

5. 包 13「闸 FAIL 续转」：闸红须带 NEXT；纯整场停/本波收口无 NEXT = 编排 bug（`next_after_fail_check`）



**禁止：** 对靶场或公网发探测请求；写利用步骤/PoC；把复盘模式当成「换皮实战开挖」。



**口令：** `复盘` / `证据包对照` / `假 covered 抽查` / `闸口演练` → 走本节，不走流水线双轨开挖。



**手法硬句（复盘对照也认）：**

- 换票成功但业务身份空 → 「空身份禁写」（见 [`术语表.md`](术语表.md)）并停枪  

- 无盾干净登录/注册口在 → 禁止「没号不开」假阴性  

- 空会话写口进落库/Controller → 一律未授权写（约束失败也算）

- `p2 FAIL` / `NEXT` / 编排 bug：释义与硬句见 [`术语表.md`](术语表.md)（禁本站 covered；必须 NEXT；无 NEXT≠整场停）

- 过盾表 `ready` → 同回合必须插登录轨 / `NEXT: 有会话`；有号/越权/凭证面收工必须指回差分 id（`diff.json#D-xx`）
...
2. **一种子闭环（§1.0.1）：** 定义见 [`术语表.md`](术语表.md)；细则认 `dig-scope` §1.0.1。禁止多种子一次搜完再挖  
...
5. **主控回合闸：** 空席未填满目标席数、或还有 leftover / 无 DONE_auth 的 cookie / pending 种子 → **本回合必须再 spawn 或跑进号脚本**，禁止纯文本收口。开场闸达标 ≠ 可以停。只有用户叫停 / 锁面簇做完 / 环境全线不可用才真停。 leftover pending 填不满空席且本种子可翻页 → 同回合翻页补池再派，禁止等最后一条 doing / yield 干等。 **leftover 真空 + 队列有 pending → 同回合必须 `NEXT: 下种子`（禁止干等、禁止只写补席）。** FOFA 0 NEW 按真空翻种。禁 covered/半条只卡本站，同回合仍要补席或翻种。  
...
> **整段调度只认同目录 `主控调度.md`。** 真源/NEXT/#4/covered 公共块以 `主控调度.md` 文首为准。 本 SKILL 不重复双轨/进号/spawn 细则。  
...
> 跑 `mark_covered.py` 前还须过下面 **suspects 覆盖率硬闸**（禁手写 `covered_hosts`）。
...
> 模板：`references/templates/suspects-template.md`。跨任务记忆：`知识库/semantic-blindspots.md` + `semantic-blindspots.auto.md`。



1. **有结构提取的站**必须落 `suspects.md`（威胁模型头 + S-xx 行）。瘦壳一眼证伪可写 `suspects: N/A 瘦壳` 一行原因。  

2. **covered 前自检**（主控跑 `mark_covered` 之前）：表单字段三问 / 接口三问 /（有会话则）认证后面 / 平凡分页参 / **信任边界（凭证作用域+半径）** —— 未勾满且无 N/A+原因 → **禁止** `mark_covered`。有会话轨 `DONE_auth` 缺 `身份=` 或 `半径=` → **禁止** `mark_covered`。  

3. **确认只打清单**：禁止按知识库模块对整站盲扫；无 S-xx 映射则先补语义疑问再测。  

4. **回灌闭环（P3）**：中危+ 确认或明确漏报 → **同回合**盲区主表或 `semantic-blindspots.auto.md` 须有对应行（`evolve_apply --auto-safe` 可写 auto 层，不改主表/rules/短表）；操典见 `知识库/回灌闭环.md`。有中危+/漏报却无盲区行 → **禁止** `mark_covered`。**auto 行 ≠ suspects 头已引用**。禁止写利用步骤/敏感实值。**令牌类（P1）**：涉及 cookie/`*_st`/passToken/OIDC/会话仓/跨租户 id 的中危+ → 同回合强制回灌，并引用「令牌绑谁/能到哪/跨租户墙」问句之一。   
...
| 微信小程序 / wxapkg / appid | `知识库/miniprogram-unpack.md`（拆包=测绘，不占席；抽出 host 进 leftover） |
...
中危、高危、严重，确认了立刻按 `vuln-report-format` 落 `报告/`。中危升链、换站认 `dig-scope` §4.3；主控开升链席（溢出席，见 `主控调度.md` / 术语表）。进不进短表只认 `hunt-iter`。未登录轨交付 = 迭代 + 缺号行（`DONE_anon`）；登录轨交付 = 迭代 + 身份行（`DONE_auth`）。测试动作上限认「约束测试红线」。禁止破坏性利用、真资损、登出用户会话。
...
python "{skill_dir}\scripts\closure_debt.py" --root "D:\SRC挖洞\{任务}_SRC挖洞"
python "{skill_dir}\scripts\mp_queue.py" --root "D:\SRC挖洞\{任务}_SRC挖洞"
python "{skill_dir}\scripts\seat_recycle.py" --root "D:\SRC挖洞\{任务}_SRC挖洞"
python "{skill_dir}\scripts\seat_recycle.py" --root "D:\SRC挖洞\{任务}_SRC挖洞" --recycle
python "{skill_dir}\scripts\rules_drift_check.py"
...
`pending_from_done.py` 扫 `DONE_anon.md` / 旧 `DONE.md` 缺号行、写 `pending_register`、打印 NEXT/REUSE（不扫 `DONE_auth`）。单元格禁 `|`（写成 ／）；状态只认 pending/doing/done/abandon。`closure_debt.py` 写 `编排/收口债.md`（leftover done ≠ covered）。`rules_drift_check.py` 对 `~/.grok/rules` 与 `D:\dsh-72stack-sec\rules`。`done_iter_scan.py` 扫三种 DONE 只列拟进/拟补，**不改短表**。进号跑 `auth_flow.py`（见 `主控调度.md`）。
