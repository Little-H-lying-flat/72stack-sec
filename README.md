...
**默认关闭**。盲测(把 confirmed 漏洞写成零上下文《AI 待复现报告》,交一个全新会话的 AI 子代理按报告字面复现)只在两种情况启动:

1. 用户明示要求盲测
2. findings 将进正式平台提交(需要"报告可复现性"的独立证明)

**范围**:只对**危害定性门(01 §3.5)受理的项**跑盲测。驳回/clean/rejected 项不是漏洞——盲测验证的是"漏洞可复现性",给它们跑是浪费额度。

**如何启动**(满足启动条件后,对执行 agent 说):

```text
对 <战役目录>/AI-REPRO-REPORT.md 跑零上下文盲测,单项分发模式(一个 finding 一次子代理)
```

或从零开始:

```text
为本轮 confirmed 写 AI 待复现报告(模板要素见 methodology/11 §7-4),然后按
templates/ai-repro-executor-prompt.md 单项分发盲测,只测反驳者受理项
```
...
| 等级 | 含义 |
|---|---|
| subagent 全盲 | 独立子代理,零共享上下文(最高) |
| 机械脚本 | 命令与判定按报告字面脚本化(中;subagent 并发受限时降级) |
| 自查 | 原作者自执行(最低,不算过闸) |

**流程**:报告(自包含:授权/账号/逐步请求/判定标准/环境前提与陷阱/超时重试/副作用声明)→ 子代理复现 → FAIL 或执行者提出报告歧义 = 报告缺陷 → 修正重测 → 才准出终稿。实战记录:testfire 6/6 PASS(抓出报告 2 缺陷)、aiwadongdemumu 9 项 11 PASS/0 FAIL(抓出 9 处,实时回填)。

**配套文件**:`methodology/11-fullauto-pipeline.md` §7-4(门闩)· `templates/ai-repro-executor-prompt.md`(执行提示词)· `templates/adversary-reviewer-prompt.md`(危害定性反驳者)
